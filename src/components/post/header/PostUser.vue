<template>
  <BaseAvatar :profileImg="userProfile?.profile_picture" :profile_id="userProfile?.id" :title="post.user?.name"
    :gender="post.user?.gender" avatarClass="avatar-50 rounded-circle" />
  <!-- <div class="user-img">
    <img
  ...
  <div class="mx-2 flex-grow-1">
    <div class="author-friends p-0 m-0">
      <span class="author-name" :class="{ 'hover-undelined': authUser.id != post.user?.id }" role="button"
        @click.prevent="
          authUser.id != post.user?.id ? sendToProfile(post.user?.id) : null
          ">{{ post.user?.name + " " + post.user?.last_name }}</span>
      <template v-if="
        post.timeline &&
      ...
      ">
        ...
        <span v-if="post.timeline.user && post.timeline.user.id != post.user?.id" class="author-name hover-undelined"
          role="button" @click.prevent="sendToProfile(post.timeline?.user?.id)">{{ post.timeline.user.name ?? ""
          }}</span>
        <span v-else-if="post.timeline.group" class="author-name hover-undelined" role="button"
          @click.prevent="sendToGroup(post.timeline.group.id)">{{ post.timeline.group.name ?? "" }}</span>
      </template>

      <template v-if="post.taggedUsers?.length">
        <span class="mx-1"> مع </span>
        <span v-for="(friend, index) in slicedTaggedFriends" :key="friend.id">
          <span class="author-name hover-undelined" role="button" @click.prevent="sendToProfile(friend.user.id)">{{
            friend.user?.name ?? "" }}</span>
          <span class="mx-1" v-if="
          ...
          <span v-if="index === slicedTaggedFriends.length - 1">
            <span v-if="post.taggedUsers?.length > slicedTaggedFriends.length" class="author-name hover-undelined"
              role="button" @click.prevent="getFriends">{{
                post.taggedUsers?.length - slicedTaggedFriends.length
              }}
              أكثر</span>
          </span>

                ...
                <div v-if="authInGroup && authInGroup.user_type != 'ambassador'">
                    ...
                    <div v-if="exception.status == 'pending'">
                            ...
                            <form @submit.prevent="submitDecision" class="post-text m-auto w-100 row">
                                <div class="form-group col-12">
                                    <select class="form-select" v-model="v$.decideForm.decision.$model" :disabled="message">
                                        ...
                                        <option value="3">اعفاء الأسبوعين القادمين [ الحالي + القادم]</option>
                            ...
                            <form @submit.prevent="submitDecision" class="post-text m-auto w-100 row">
                                <div class="form-group col-12">
                                    <select class="form-select" v-model="v$.decideForm.decision.$model" :disabled="message">
                                        ...
                                        <option value="2">اعفاء الأسبوع القادم</option>
                                        <option value="3">اعفاء الأسبوعين القادمين [ الحالي + القادم]</option>
                                        <option value="4">اعفاء ثلاثة أسابيع [ الحالي + القادم + الذي
                                            يليه]</option>
                                        <option value="0">رفض</option>
                                    </select>
                                    <small style="color:red;" v-if="v$.decideForm.decision.$error">
                        ...
                        <div class="d-inline-flex justify-content-center alert alert-success mt-2 w-75" v-else>
                            <h5>
                                يتطلب هذا الاجراء موافقة موجه المجموعة
                            </h5>
                        </div>
                    </div>
                    ...
                    <div class="d-inline-flex justify-content-center alert alert-success mt-2 w-75" v-else>
                        <h5>
                            تم اتخاذ الاجراء مسبقًا
                        </h5>
                    </div>
                </div>
                <div class="d-inline-flex justify-content-center w-100"
                    v-if="(authInGroup.user_id == auth.id) && (exception.status == 'accepted' || exception.status == 'pending')">
    ...
    methods: {
        async submitDecision() {
            ...
            if (!this.v$.decideForm.$invalid) {
                ...
                this.loader = true;
                try {
                    const response = await exceptionService.updateStatus(this.exception.id, this.decideForm);
                    this.loader = false;
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    .then((willDelete) => {
                        if (willDelete.isConfirmed) {
                            this.loader = true
                            exceptionService.cancelException(exception_id)
                                .then(response => {
                                    swalWithBootstrapButtons.fire({
                                        title: 'تم الاغاء',

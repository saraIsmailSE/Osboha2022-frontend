  ...
  <div class="group-member d-flex align-items-center mt-md-0 mt-3 w-100">
    <div class="iq-media-group me-3 col-6" v-if="members.length > 0">
      <router-link class="iq-media" :to="{ name: 'group.group-members', params: { group_id: group_id } }">
        <img src="@/assets/images/main/more_users.png" alt="profile-img"
          class="img-fluid avatar-40 rounded-circle" :title="`عرض الكل`" />
      </router-link>
      <router-link class="iq-media" v-for="member in members.slice(0, 5)" :key="member.id"
        :to="{ name: 'user.profile', params: { user_id: member.id } }">
        <img v-if="member.user_profile.profile_picture"
          :src="resolve_porfile_img('60x60', member.user_profile.profile_picture, member.user_profile.id)"
          alt="profile-img" class="img-fluid avatar-40 rounded-circle" :title="member.name" />

        <img v-else :src="resolve_porfile_img('60x60', 'ananimous_' + member.gender + '.png', 'ananimous')"
          alt="profile-img" :title="member.name" class="img-fluid avatar-40 rounded-circle">
      </router-link>
    </div>

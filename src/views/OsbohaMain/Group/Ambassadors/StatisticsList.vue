        ...
        <ul class="todo-task-list p-0 m-0">
          <li v-for="(category, index1) in categoryList" :key="index1">
            <a class="d-flex align-items-center justify-content-between">
              ...
              <div class="d-inline p-2 text-light float-end rounded" :style="`background-color:${category.color}`">
                {{ category.number }}
              </div>
            </a>
          </li>
...
export default {
  ...
  created() {
    ...
    this.chartData.datasets[0].data[3] = this.categoryList[3].number;
  },
  props: ["categoryList"],
  components: {
    Doughnut,
  ...
  data() {
    return {
      ...
      chartData: {
        labels: ["100", "90", "صفر", "تجميد"],
        datasets: [
          {
            label: "انجاز الأسبوع",
            data: [20, 7, 3],
            backgroundColor: ["#22803f", "#192f36", "#831018", "#223e7f"],
            hoverOffset: 4,
          },
        ],
      },

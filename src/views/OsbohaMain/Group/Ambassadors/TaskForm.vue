...
export default {
  ...
  methods: {
    ...
    saveTask (item) {
      item.id = this.$store.getters['Ambassadros/selectedProjectState'].id
      item.project_id = this.$store.getters['Ambassadros/selectedProjectState'].id
      item.category_id = item.category_id.code
      item.user_id = item.user_id.code
      this.$store.dispatch('Ambassadros/addTaskAction', item)

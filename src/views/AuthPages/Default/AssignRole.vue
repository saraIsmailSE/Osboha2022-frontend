        ...
        <div class="sign-in-from">
          ...
          <form class="mt-2" @submit.prevent="onSubmit()">
            <div class="form-group">
              ...
              <small style="color: red" v-if="v$.form.email.$error">الرجاء ادخال ايميل صالح</small>
            </div>
            <div class="form-group">
              ...
              <small style="color: red" v-if="v$.form.role.$error">Please select a role.</small>
            </div>
...
export default {
  ...
  methods: {
     async  getRoles(){
           const a = JSON.parse(localStorage.getItem("user")).user.roles;
           const biggestRole = a.reduce((acc, obj) =>obj.id < acc ? obj : acc, Infinity);
   ...
   async  onSubmit() {
      this.v$.$validate();
      if (!this.v$.$error) {
        const res = await authService.assignRole({email:this.form.email,role:this.form.role})
